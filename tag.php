...
<!-- 标签信息通栏 -->
<div class="bg-gradient-to-r from-green-500 to-blue-600 text-white hidden md:block">
    <div class="max-w-1500 mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex items-center">
            <i class="fas fa-tag text-4xl mr-6"></i>
            <div>
                <h1 class="text-4xl md:text-5xl font-bold mb-3"><?php single_tag_title(); ?></h1>
                <div class="text-green-100 text-xl mb-4">
                    <?php 
                    $tag_desc = tag_description();
                    if ($tag_desc) {
                        echo wp_kses_post($tag_desc);
                    } else {
                        echo '探索标签「' . single_tag_title('', false) . '」下的相关内容，这些文章都围绕着这个主题展开，为您带来多角度的深入分析和独特见解。';
                    }
                    ?>
                </div>
                <div class="flex items-center text-green-100">
                    <i class="fas fa-file-alt mr-2"></i>
                    <span class="text-lg">包含 <?php echo esc_html($wp_query->found_posts); ?> 篇相关文章</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex-1 lg:w-2/3">
        ...
        <?php else : ?>
            ...
            <div class="bg-white rounded-xl shadow-lg p-12 text-center">
                ...
                <h2 class="text-2xl font-bold text-gray-900 mb-2">该标签下暂无文章</h2>
                <p class="text-gray-600 mb-6">这个标签还没有关联任何文章。</p>
